...
func (rc *RestoreController) restoreTables(ctx context.Context) error {
	...
	// actually, this task is quite fast, but we use multi routine to try to run multi
	...
	go func() {
		defer wg.Done()
		defer close(importChan)
		if err := rc.restoreEngines(ctx2, restoreChan, importChan); err != nil {
			restoreErr.Set(err)
			return
		}
	}()
	...
	go func() {
		...
		for task := range postProcessChan {
			...
			go func(t *importedTable) {
				defer wg.Done()
				err := t.tr.postProcess(ctx2, rc, t.cp)
				if err != nil {
					restoreErr.Set(err)
				}
				task.tr.restoreLogTask.End(zap.ErrorLevel, err)
			}(task)
		}
	}()
...
func (rc *RestoreController) restoreEngines(
	ctx context.Context,
	taskChan <-chan *tableEngine,
	importChan chan *importEngine,
) error {
	var wg sync.WaitGroup
	restoreCtx, cancel := context.WithCancel(ctx)
	defer cancel()
	errorChan := make(chan error, rc.cfg.App.TableConcurrency)
	...
	for i := 0; i < concurrency; i++ {
		...
		go func(w *worker.Worker) {
			...
			for task := range importChan {
				...
				if task.triggerCompact && rc.cfg.PostRestore.Level1Compact &&
...
type TableRestore struct {
	...
	logger               log.Logger
	engineCount          int32
	finishRestoreEngine  int32
	finishedImportEngine int32

	restoreLogTask *log.Task
}
