...
func (s *cpSQLSuite) TestNormalOperations(c *C) {
	...
	insertChunkStmt.
		ExpectExec().
		WithArgs("`db1`.`t2`", 0, "/tmp/path/1.sql", 0, mydump.SourceTypeSQL, 0, "", 102400, []byte("null"), 12, 102400, 1, 5000, 1234567890).
	...
	s.mock.
		...
		WillReturnRows(
			sqlmock.NewRows([]string{
				...
				AddRow(
					0, "/tmp/path/1.sql", 0, mydump.SourceTypeSQL, 0, "", 102400, "[]",
...
func (s *cpSQLSuite) TestDump(c *C) {
	...
	s.mock.
		...
		WillReturnRows(
			sqlmock.NewRows([]string{
				...
				"`db1`.`t2`", "/tmp/path/1.sql", 0, mydump.SourceTypeSQL, mydump.CompressionNone, "", 102400, "[]",
				55904, 102400, 681, 5000,
	...
	c.Assert(csvBuilder.String(), Equals,
		"table_name,path,offset,type,compression,sort_key,size,columns,pos,end_offset,prev_rowid_max,rowid_max,kvc_bytes,kvc_kvs,kvc_checksum,create_time,update_time\n"+
			"`db1`.`t2`,/tmp/path/1.sql,0,3,0,,102400,[],55904,102400,681,5000,4491,586,486070148917,2019-04-18 02:45:55 +0000 UTC,2019-04-18 02:45:55 +0000 UTC\n",
	)
